...
export function DosDemo() {
  ...
  const [attackType, setAttackType] = useState<'aggressive' | 'batched' | null>(null);
  ...
  const averageServerProcessingTime = serverProcessingTimes.length > 0
    ? serverProcessingTimes.reduce((a, b) => a + b, 0) / serverProcessingTimes.length
    : 0;
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="space-y-2">
        ...
        <p>Average server processing time: {averageServerProcessingTime.toFixed(2)}ms</p>
        ...
        <div className="flex gap-2">
          <Button
            ...
            variant="destructive"
          >
            {isLoading && attackType === 'aggressive' ? "Degrading..." : "Degrade Server"}
          </Button>
          <Button
            ...
            variant="secondary"
          >
            {isLoading && attackType === 'batched' ? "Processing..." : "Normal Processing"}
          </Button>
        </div>
      </div>
    </div>
